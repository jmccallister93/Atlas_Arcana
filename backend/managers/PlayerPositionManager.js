class PlayerPositionManager {
  ...
  async updatePlayerPosition(io, sessionId, newPlayerPosition) {
    ...
    try {
        ...
        console.log("Current player positions before update:", parsedSessionData.playerPositions);

        // Check if the player position for the user already exists
        const existingIndex = parsedSessionData.playerPositions.findIndex(
            position => position.playerUsername === newPlayerPosition.playerUsername
        );
        ...
        console.log("Updated player positions:", parsedSessionData.playerPositions);
