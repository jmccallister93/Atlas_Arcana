...
const GameTurnManager: React.FC<GameTurnManagerProps> = ({}) => {
  ...
  const currentPhase = useGameStatePart((state) => state.currentPhase as string)
  const currentPlayerTurn = useGameStatePart((state) => state.currentPlayerTurn as string )
  const setupPhase = useGameStatePart((state) => state.setupPhase as boolean)
  const turnsCompleted = useGameStatePart((state) => state.turnsCompleted as number)
  ...
  const advancePhase = () => {
    ...
    if (setupPhase) {
      ...
      if (nextPhase === phaseOrder[0]) {
        ...
        if (nextPlayerIndex === 0 && currentPhaseIndex === phaseOrder.length - 1) {
